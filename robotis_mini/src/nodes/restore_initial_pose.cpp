#include "robotis_mini/restore_initial_pose.hpp"
    ...
    : Node("restore_initial_pose", opts)
{
    declare_parameter<std::vector<std::string>>(
      "joint_names", rcl_interfaces::msg::ParameterDescriptor{}
    );
    declare_parameter<std::vector<double>>(
      "initial_positions", rcl_interfaces::msg::ParameterDescriptor{}
    );
    ...
    if (joint_names_.empty() || initial_positions_.size() != joint_names_.size()) {
        RCLCPP_FATAL(
          get_logger(),
          "Params invalid: joint_names or initial_positions mismatch."
        );
...
void restore_initial_pose::execute_goal(const std::shared_ptr<GoalHandle> gh)
{
    ...
    // optional tolerances can be set here; controller enforces tolerances on success.

    auto send_opts = rclcpp_action::Client<FJT>::SendGoalOptions();
    send_opts.feedback_callback =
        [this, gh, feedback](std::shared_ptr<rclcpp_action::ClientGoalHandle<FJT>>,
